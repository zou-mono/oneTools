...
class Autocad:
    ...
    def OpenFile(self, path, bReadOnly=True):
        """
    ...
    def SaveAsFile(self, path, fileType=60):
        """
        save as file
        """
        self.acad.ActiveDocument.SaveAs(path, fileType)

    def Close(self, bSaveChanges):
        """
        Close current file
        :return:
        """
        try:
            self.acad.ActiveDocument.Close(bSaveChanges)
        except:
            pass
    ...
    def GetCurrentUCS(self):
        ...
        if self.acad.ActiveDocument.GetVariable('ucsname') == '':
            ...
            xAxisPnt = self.acad.ActiveDocument.Utility.TranslateCoordinates(
                ArrayTransform(self.acad.ActiveDocument.GetVariable('ucsxdir')),
                win32com.client.constants.acUCS, win32com.client.constants.acWorld, 0)
            xAxisPnt = ArrayTransform(xAxisPnt)
            yAxisPnt = self.acad.ActiveDocument.Utility.TranslateCoordinates(
                ArrayTransform(self.acad.ActiveDocument.GetVariable('ucsydir')),
                win32com.client.constants.acUCS, win32com.client.constants.acWorld, 0)
