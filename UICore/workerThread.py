...
class crawlTilesWorker(QtCore.QObject):
    crawl = pyqtSignal(str, int, int, int, float, float, float, float, float, int, str, str, str)
    crawlAndMerge = pyqtSignal(str, int, int, int, float, float, float, float, float, int, str, bool, str, str, str, str)
    ...
    def crawlTiles(self, url, level, x0, y0, xmin, xmax, ymin, ymax, resolution, tile_size, api_token, subscription_token, output_path):
        # count = 0
        # while count < 50:
        #     time.sleep(1)
        #     print("B Increasing")
    ...
    def crawlAndMergeTiles(self, url, level, x0, y0, xmin, xmax, ymin, ymax, resolution, tile_size, pixelType, bCompression,
                           output_path, api_token, subscription_token, merged_file):
        crawl_tilemap(url, level, x0, y0, xmin, xmax, ymin, ymax, resolution, tile_size, api_token, subscription_token, output_path)
